...
class SSHConnection:
    ...
    def __init__(self, host=g_HostName, port=g_Port, username=g_UserName, pwd=g_Password):
        self.host = host
        self.port = port
...
def main():
    ssh = SSHConnection()
    ssh.cmd('ls -lah')  # 执行ls -lah命令,并执行run.sh脚本
    ...
    ssh.download('/home/icttw/systemlog/zfy.zip', 'zfy.zip')  # 将远端服务器的/home/python/Desktop目录下的1.p下载到本地的test目录下并改名为test.py
    # ssh.close()  # 关闭连接
